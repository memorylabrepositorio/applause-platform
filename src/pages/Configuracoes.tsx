  ...
  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-slate-950 text-slate-100">
        ...
        <Link to="/sdr" className="text-indigo-400 hover:text-indigo-300">← Voltar</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100 sm:p-8">
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Comportamento do agente e integrações da plataforma</p>
        </div>
        <Link to="/sdr" className="text-sm text-slate-400 hover:text-slate-200">
          ← Voltar pro SDR
        </Link>
      </header>
      ...
      <section className="mb-6 rounded-xl border border-slate-800 bg-slate-900 p-4">
        <p className="mb-1 text-sm font-medium text-slate-300">Comportamento do agente</p>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-slate-500">Nome do agente</label>
            <input
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-500">Tom de voz</label>
            <select
      ...
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-slate-500">URL da instância</label>
            <input
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-500">Nome da instância</label>
            <input
      ...
      >
        <div>
          ...
          <input
            ...
            className="w-full max-w-sm rounded-md border border-slate-700 bg-slate-800 px-3 py-1.5 text-sm"
          />
        </div>
        <SecretField
          label="Access Token"
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          className={`rounded-full border px-3 py-1 text-xs ${
            ativo ? "border-emerald-700 bg-emerald-950 text-emerald-300" : "border-slate-700 text-slate-400"
          }`}
        >
    ...
    <div className="mt-2 border-t border-slate-800 pt-3">
      ...
      <div className="flex flex-wrap gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-md border border-slate-700 bg-slate-800 px-3 py-1.5 text-sm"
        />
        <button
          onClick={handleSave}
